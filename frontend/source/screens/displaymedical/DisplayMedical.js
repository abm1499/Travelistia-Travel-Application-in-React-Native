...
const DisplayMedical = ({navigation}) => {
  ...
  useEffect(() => {
    initialRegion.latitude != 33.6844 ? getplaces('pharmacy') : null;
  }, [initialRegion]);
  ...
  const getplaces = async value => {
    if (initialRegion.latitude != 33.6844) {
      const response = await fetch(
        ...
        }&radius=5000&type=${value}&key=${'AIzaSyAHX4k9xVfM4T5HS7LP8lP92_XomVsi06U'}`,
      );
